...
import id.co.qualitas.erajaya.adapter.CreateIssuingAdapter;
import id.co.qualitas.erajaya.constants.Constants;
import id.co.qualitas.erajaya.databinding.ActivityCreateIssuingBinding;
...
public class CreateIssuingActivity extends BaseActivity {
    private ActivityCreateIssuingBinding binding;
    ...
    private CreateIssuingAdapter adapter;
    private List<Issuing> listIssuing = new ArrayList<>();
    private int PARAM = 0;
    private String ean;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityCreateIssuingBinding.inflate(getLayoutInflater());
...
//        binding.spPlant.setAdapter(dataPlantAdapter);
//
//        ArrayAdapter<String> dataSlocAdapter = new ArrayAdapter<String>(this, android.R.layout.simple_spinner_item, listSloc);
//        dataSlocAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
//        binding.spSloc.setAdapter(dataSlocAdapter);
//
//        ArrayAdapter<String> dataMovTypAdapter = new ArrayAdapter<String>(this, android.R.layout.simple_spinner_item, listMovTyp);
//        dataMovTypAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
//        binding.spMovType.setAdapter(dataMovTypAdapter);
...
//        });
    ...
    public void showDialogHeader() {
        if (bottomSheetDialog == null || !bottomSheetDialog.isShowing()) {
            bottomSheetDialog = Helper.showBottomSheetDialog(this, R.layout.bs_header_issuing, (itemView, bottomSheet) -> {
                ...
                btnSubmit.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        ...
                        if (edtRecvNo.getText().toString().isEmpty()) {
                            Snackbar.make(view, "Issuing doc number cannot empty", Snackbar.LENGTH_SHORT).show();
                        }
...
//                        }
                            ...
                            new AlertDialog.Builder(CreateIssuingActivity.this)
                                    .setMessage("Are you sure to submit this document?")
                                    .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                                        @Override
                                        public void onClick(DialogInterface dialog, int which) {
                                            listIssuing = new ArrayList<>();
                                            for (Material mat : listMaterial) {
                                                Issuing issuing = new Issuing(edtRecvNo.getText().toString(), mat.getMaterialNumber()
    ...
    private class RequestUrl extends AsyncTask<Void, Void, WSMessage> {
        ...
        protected WSMessage doInBackground(Void... voids) {
            try {
                if (PARAM == 0) {
                    String URL_ISSUING = Constants.API_INSERT_ISSUING;

                    final String url = Helper.getItemParam(Constants.URL).toString().concat(URL_ISSUING);

                    return (WSMessage) Helper.postWebserviceWithBody(url, WSMessage.class, listIssuing, getApplicationContext());
                } else {
                    String URL_MATERIAL = Constants.API_GET_MATERIAL_ISSUING + Constants.QUESTION + "ean=" + ean;
    ...
    private void setDataItem() {
        if (Helper.getItemParam(Constants.LIST_MATERIAL) != null) {
            ...
            if (listMaterial.size() != 0) {
                binding.rcItem.setVisibility(View.VISIBLE);
                adapter = new CreateIssuingAdapter(CreateIssuingActivity.this, listMaterial);
                RecyclerView.LayoutManager layoutManager = new LinearLayoutManager(CreateIssuingActivity.this);
